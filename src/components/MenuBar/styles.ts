...
import { Home, Notifications, Email, FavoriteBorder, Person, ExitToApp, LocationOn, Search, Rocketseat } from '../../styles/icons'
...
export const MenuButton = styled.button`
  ...
  @media (min-width: 1280px){
    >span{
      display: inline; //a partir da ai ele vai aparecer
      margin-left: 19px;
  ...
  & + button:last-child{
    ...
    @media (min-width: 1280px) {
      width: 100%;
      height: unset; //pra volta a letra no botao9
...
export const Botside = styled.div`
  margin-top: 20px; //centrallizar os itens 
  display: flex;
  align-items: center;
`;
